...
def sharpe(returns, freq=30, rfr=0,trade_lenghth=365): #rfr无风险回报
    """ Given a set of returns, calculates naive (rfr=0) sharpe (eq 28). """
    return (np.sqrt(freq) * np.mean(returns - rfr + eps)) / np.std(returns - rfr + eps)

    #print(trade_lenghth)
    #return (  np.mean(returns - rfr + eps) / np.std(returns - rfr + eps))/np.sqrt(freq/trade_lenghth)
...
class DataGenerator(object):
    ...
    def reset(self):
        self.step = 0
        #print("data shape is ：  ",  self._data.shape[1])
        ...
        if self.start_date is None:
            ...
            #print('cur steps is:  ',  self.steps)
            #print('cur high date is   ', self._data.shape[1] - self.steps)
        else:
...
class PortfolioEnv(gym.Env):
    ...
    def plot(self):
        ...
        sharpe_ratio = sharpe(df_info.rate_of_return,trade_lenghth=df_info.shape[0])
